...
# Point plot
Rcatch22_plot_sd500 <- ggplot(features_sd500_long, aes(y = names, x = value, color = as.numeric(parameters))) + 
  geom_point() + 
  theme_minimal() + 
  labs(y = "Names", x = "Values", color = "Param") + 
  ggtitle("Comparative Visualization of Simple Decimation for DF500") +
...
# normalize columns 1, 2, 5, 10, 20
cols_to_normalize <- c('1', '2', '5', '10', '20')
features_pc500[cols_to_normalize] <- scale(features_sd500[cols_to_normalize])
...
# Point plot
Rcatch22_plot_pc500 <- ggplot(features_pc500_long, aes(y = names, x = value, color = as.numeric(parameters))) + 
  geom_point() + 
  theme_minimal() + 
  labs(y = "Names", x = "Values", color = "Param") + 
  ggtitle("Comparative Visualization of Percentage Change for DF500") +
  theme(axis.title.y = element_blank(), axis.title.x = element_blank())
...
grid.arrange(patchworkGrob(Rcatch22_plot_sd500 + Rcatch22_plot_pc500), left = "Names", bottom = "Values")
