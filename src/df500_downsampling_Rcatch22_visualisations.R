...
# Point plot
Rcatch22_plot_sd600 <- ggplot(features_sd600_long, aes(y = names, x = value, color = as.numeric(parameters))) + 
  geom_point() + 
  theme_minimal() + 
  labs(y = "Names", x = "Values", color = "Param") + 
  ggtitle("Comparative Visualization of Simple Decimation for DF600") +
...
# normalize columns 1, 2, 5, 10, 20
cols_to_normalize <- c('1', '2', '5', '10', '20')
features_pc600[cols_to_normalize] <- scale(features_sd600[cols_to_normalize])
...
# Point plot
Rcatch22_plot_pc600 <- ggplot(features_pc600_long, aes(y = names, x = value, color = as.numeric(parameters))) + 
  geom_point() + 
  theme_minimal() + 
  labs(y = "Names", x = "Values", color = "Param") + 
  ggtitle("Comparative Visualization of Percentage Change for DF600") +
  theme(axis.title.y = element_blank(), axis.title.x = element_blank())
...
grid.arrange(patchworkGrob(Rcatch22_plot_sd600 + Rcatch22_plot_pc600), left = "Names", bottom = "Values")
